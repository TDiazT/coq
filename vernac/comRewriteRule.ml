...
let interp_rule ~poly (udecl, lhs, rhs: Constrexpr.sort_poly_decl_expr option * _ * _) =
  let env = Global.env () in
  ...
  let flags = { Pretyping.no_classes_no_fail_inference_flags with polymorphic = poly } in
  let evd', rhs =
