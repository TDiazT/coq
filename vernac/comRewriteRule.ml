...
let interp_rule ~sort_poly (udecl, lhs, rhs: Constrexpr.universe_decl_expr option * _ * _) =
  let env = Global.env () in
  ...
  let poly = PolyFlags.make ~univ_poly:true ~cumulative:false ~collapse_sort_variables:true in (* FIXME:  originally sort_poly *)
