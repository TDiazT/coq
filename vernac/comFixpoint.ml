...
let do_mutually_recursive ?pm ~refine ~program_mode ?(use_inference_hook=false) ?scope ?clearbody ~kind ~poly ~sort_poly ?typing_flags ?user_warns ?using (rec_order, fixl)
  ...
  | Some pm ->
    ...
    Evd.check_poly_decl_early ~poly ~sort_poly ~with_obls:true sigma udecl (bodies @ fixtypes);
  ...
  | None ->
    ...
    with Option.IsNone ->
      ...
      Evd.check_poly_decl_early ~poly ~sort_poly ~with_obls:false sigma udecl (Option.List.flatten bodies @ fixtypes);
      let possible_guard = (possible_guard, fixrs) in
