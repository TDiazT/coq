...
let finalize_def_class ~poly env sigma ~params ~sort ~projtyp =
  let sigma, (params, sort, typ, projtyp) =
    Evarutil.finalize ~abort_on_undefined_evars:false ~to_type:(PolyFlags.collapse_sort_variables poly) sigma (fun nf ->
...
let build_named_proj ~primitive ~flags ~poly ~univs ~uinstance ~kind env paramdecls
  ...
  let univs = check_add_elimination_constraints ~primitive ~poly env univs record_q proj_typ in
