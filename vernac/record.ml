...
let check_add_elim_constraint ~primitive_proj ~sort_poly env sigma record_quality fld_sort =
  (* Only adding elimination constraints for primitive projections
    if the sort poly flag is enabled, for now *)
  if primitive_proj && sort_poly then
      ...
      (fun (env, sigma, uimpls, locs, params, impls_env) fld_notation d ->
        let sigma, (i, b, t), impl, loc = match d with
          | Vernacexpr.AssumExpr({CAst.v=id; loc}, bl, t) ->
            ...
            let sigma = check_add_elim_constraint ~primitive_proj ~sort_poly env sigma record_quality fld_sort in
...
let check_add_elimination_constraints ~primitive ~sort_poly env univs record_quality proj_typ =
...
let build_named_proj ~primitive ~flags ~sort_poly ~univs ~uinstance ~kind env paramdecls
  ...
  let univs = check_add_elimination_constraints ~primitive ~sort_poly env univs record_q proj_typ in
...
let build_proj mib indsp primitive x rp lifted_fields paramdecls paramargs record_quality ~sort_poly ~uinstance ~kind
...
let declare_projections indsp ~kind ~inhabitant_id flags ~sort_poly ?fieldlocs fieldimpls =
  ...
  let (_, _, _, _, canonical_projections, _) =
    List.fold_left4
      (build_proj mib indsp primitive x rp lifted_fields paramdecls paramargs record_quality ~sort_poly ~uinstance ~kind)
