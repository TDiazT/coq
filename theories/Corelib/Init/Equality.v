...
Class Has_refl@{sa se;la le} (eq : forall A : 𝒰@{sa;la}, A -> A -> 𝒰@{se;le})
:= refl : forall A x, eq A x x.

(* This class register a Martin-Löf like elimination principle *)

Class Has_J@{sa se sp;la le lp} (eq : forall A : 𝒰@{sa ; la}, A -> A -> 𝒰@{se;le})
  (Has_refl : Has_refl eq) :=
  J_eliminator : forall (A : 𝒰@{sa ; la}) (x : A) (P : forall y : A, eq A x y -> 𝒰@{sp ; lp}),
    P x (refl A x) -> forall y e, P y e.

(* This class is for forward dependent rewriting *)

Class Has_J_r@{sa se sp;la le lp} (eq : forall A : 𝒰@{sa ; la}, A -> A -> 𝒰@{se;le})
  (Has_refl : Has_refl eq) :=
  J_r_eliminator: forall (A : 𝒰@{sa ; la}) (x : A) (P : forall y : A, eq A y x -> 𝒰@{sp ; lp}),
    P x (refl A x) -> forall y e, P y e.

(* Those two classes are for dependent rewriting in an hypotesis *)

Class Has_J_forward@{sa se sp;la le lp} (eq : forall A : 𝒰@{sa ; la}, A -> A -> 𝒰@{se;le})
  (Has_refl : Has_refl eq) :=
  J_forward : forall (A : 𝒰@{sa ; la}) (x : A) (P : forall y : A, eq A x y -> 𝒰@{sp ; lp}) y e,
    P y e -> P x (refl A x).

Class Has_J_r_forward@{sa se sp;la le lp} (eq : forall A : 𝒰@{sa ; la}, A -> A -> 𝒰@{se;le})
  (Has_refl : Has_refl eq) :=
  J_r_forward : forall (A : 𝒰@{sa ; la}) (x : A) (P : forall y : A, eq A y x -> 𝒰@{sp ; lp}) y e,
    P y e -> P x (refl A x).

(* Those two classes are for non-dependent rewriting *)

Class Has_Leibniz@{sa se sp;la le lp} (eq : forall A : 𝒰@{sa ; la}, A -> A -> 𝒰@{se;le}) :=
  leibniz : forall (A : 𝒰@{sa ; la}) (x : A) (P : A -> 𝒰@{sp ; lp}), P x -> forall y, eq A x y -> P y.

Class Has_Leibniz_r@{sa se sp;la le lp} (eq : forall A : 𝒰@{sa ; la}, A -> A -> 𝒰@{se;le}) :=
  leibniz_r : forall (A : 𝒰@{sa ; la}) (x : A) (P : A -> 𝒰@{sp ; lp}), P x -> forall y, eq A y x -> P y.
...
Definition J_no_dep@{s s' sp;l l' lp} {eq} {refl} (eqr : Has_J@{s s' sp;l l' lp} eq refl) :
  forall (A : 𝒰@{s ; l}) (x : A) (P : A -> 𝒰@{sp ; lp}), P x -> forall y (e : eq A x y), P y :=
...
Section ap.
  ...
  Context {eq : forall A : 𝒰@{sa;la}, A -> A -> 𝒰@{se;le} }
          {eq' : forall A : 𝒰@{sb; lb}, A -> A -> 𝒰@{se';le'} }
          {_refl: Has_refl@{sb se';lb le'} eq'}
          {_leibniz: Has_Leibniz@{sa se se';la le le'} eq}.

  Definition ap [A : 𝒰@{sa;la}] [B:𝒰@{sb;lb}] (f : A -> B) [x y : A] (e : eq _ x y) : eq' _ (f x) (f y) :=
...
Definition Has_J_Has_J_forward@{sa se sp;la le lp} eq Has_refl
  ...
  forall (A : 𝒰@{sa ; la}) (x : A) (P : forall y : A, eq A x y -> 𝒰@{sp ; lp}) y e,
      P y e -> P x (refl A x) :=
...
Definition Has_J_r_Has_J_r_forward@{sa se sp;la le lp} eq Has_refl
  ...
  forall (A : 𝒰@{sa ; la}) (x : A) (P : forall y : A, eq A y x -> 𝒰@{sp ; lp}) y e,
      P y e -> P x (refl A x) :=
