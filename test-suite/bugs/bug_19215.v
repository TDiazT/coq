...
Polymorphic Inductive eq@{s s';u v|} (A:𝒰@{s;u}) (x:A) : A -> 𝒰@{s';v} :=
    eq_refl : x = x :>A
...
Polymorphic Definition eq_elim@{s s' ; u v w |} [A:𝒰@{s;u}] [x:A]
  (P : forall a : A, x = a :> A -> 𝒰@{s';w}) :
...
Polymorphic Definition eq_singleton@{s s' ; u v|} [A:𝒰@{s;u}] [x:A]
  (P : forall a : A, (eq@{s Prop;u Set} x a) -> 𝒰@{s';v}) :
...
Register Scheme eq_ind as ind_nodep for eq.
  ...
  Section equality.
    ...
    Theorem eq_sym@{s;u|} (A : 𝒰@{s;u}) (x y : A) : x = y -> y = x.
    Proof.
      destruct 1; trivial.
    Defined.
    ...
    Theorem eq_trans@{s;u|} (A : 𝒰@{s;u}) (x y z : A) : x = y -> y = z -> x = z.
    Proof.
      destruct 2; trivial.
    Defined.
    ...
    Theorem f_equal@{s s';u v|} (A : 𝒰@{s;u}) (B : 𝒰@{s';v})
