...
Module Conversion.
  ...
  Inductive Box@{s;u|} (A:𝒰@{s;u}) := box (_:A).

  Definition t1@{s;u|} (A:𝒰@{s;u}) (x y : A) := box _ x.
  Definition t2@{s;u|} (A:𝒰@{s;u}) (x y : A) := box _ y.

  Definition t1'@{s;u|} (A:𝒰@{s;u}) (x y : A) := x.
  Definition t2'@{s;u|} (A:𝒰@{s;u}) (x y : A) := y.
  ...
  Definition ignore@{s;u|} {A:𝒰@{s;u}} (x:A) := tt.
  Definition unfold_ignore@{s;u|} (A:𝒰@{s;u}) : ignore (t1 A) = ignore (t2 A) := eq_refl.

  Definition t (A:SProp) := Eval lazy in t1 A.

  Axiom v@{s; |} : forall (A:𝒰@{s;Set}), bool -> A.
...
Module Inductives.
  ...
  Fail Inductive foo3@{s; |} (A:𝒰@{s;Set}) := Foo3 : A -> foo3 A.
  Inductive foo3@{s; |Type->s} (A:𝒰@{s;Set}) := Foo3 : A -> foo3 A.
  Check foo3_rect.

  Fail Inductive foo4@{s;u v|v < u} : 𝒰@{v} := C (_:𝒰@{s;u}).

  Inductive foo5@{s; |} (A:𝒰@{s;Set}) : Prop := Foo5 (_ : A).
  Definition foo5_ind'@{s; |} : forall (A : 𝒰@{s;Set}) (P : Prop), (A -> P) -> foo5 A -> P
    := foo5_ind.
  ...
  Definition foo6_qsort_rect@{s;u|} (P:foo6 -> 𝒰@{s;u})
    (H : P Foo6)
    (f : foo6@{s;})
    : P f
    := match f with Foo6 => H end.

  Fail Definition foo6_2qsort_rect@{s s';u|} (P:foo6 -> 𝒰@{s;u})
    (H : P Foo6)
    (f : foo6@{s';})
    : P f
    := match f with Foo6 => H end.
  ...
  Inductive foo7@{s; |} : 𝒰@{s;Set} := Foo7_1 | Foo7_2.
  ...
  Fail Record R1@{s; |} : 𝒰@{s;Set} := {}.

  (* the Type instantiation may not be primitive *)
  Fail Record R2@{s; |} (A:SProp) : 𝒰@{s;Set} := { R2f1 : A }.

  (* R3@{SProp Type|} may not be primitive  *)
  Fail Record R3@{s s'; |} (A:𝒰@{s;Set}) : 𝒰@{s';Set} := { R3f1 : A }.
  ...
  Record R6@{s; |Type->s} (A:𝒰@{s;Set}) := { R6f1 : A; R6f2 : nat }.
  ...
  #[projections(primitive=no)] Record R7@{s; |Type -> s} (A:𝒰@{s;Set}) := { R7f1 : A; R7f2 : nat }.
  ...
  Inductive seq@{s;u|} (A:𝒰@{s;u}) (a:A) : A -> Prop := seq_refl : seq A a a.
  ...
  Inductive sexists@{s;u|} (A:𝒰@{s;u}) (B:A -> Prop) : Prop
    := sexist : forall a:A, B a -> sexists A B.

  (* we can eliminate to Prop *)
  Check sexists_ind.

  Inductive sigma3@{s s' s'';u v| } (A:𝒰@{s;u}) (P:A -> 𝒰@{s';v}) :
    𝒰@{s'';max(u,v)} :=
  ...
  (* This is now invalid since Type does not eliminate to arbitrary sorts by default *)
  Fail Definition π1@{s s';u v|} {A:𝒰@{s;u}} {P:A -> 𝒰@{s';v}} (p : sigma3@{_ _ Type;_ _} A P) : A :=
    match p return A with exist3 a _ => a end.
  Definition π1@{s s';u v|Type -> s} {A:𝒰@{s;u}} {P:A -> 𝒰@{s';v}} (p : sigma3@{_ _ Type;_ _} A P) : A :=
    match p return A with exist3 a _ => a end.
  (* s s' ; u v |= Type -> s *)
End Inductives.
